...
## What link is required for submission?

There are two different links:

1. **Local deployment URL (from this project)**  
   Example: `http://localhost/<deployment-id>`  
   This proves your local pipeline works.

2. **Required Brimble deploy URL (JD requirement)**  
   Example: `https://<your-app>.brimble.io` (or the URL Brimble gives you)  
   This is a separate deploy on Brimble and is required for submission.

You should submit **both**:
- the repo (this project)
- one public Brimble deploy link + brief honest feedback
...
## Deploy your first app in this pipeline
...
6. When status is `running`, open the URL from the card.

GitHub subdirectory URLs are supported, for example:

`https://github.com/<owner>/<repo>/tree/<branch>/sample-app`

## Generate the required Brimble link (JD)

Deploy any small app on Brimble (can be hello-world).

1. Sign in to [Brimble](https://www.brimble.io/).
2. Create a new app and import a repo (or use their preferred flow).
3. Complete deploy.
4. Copy the public URL Brimble gives you.
5. Add that URL in your submission notes.

Also include 1-2 short paragraphs of candid feedback:
- what worked
- what felt confusing
